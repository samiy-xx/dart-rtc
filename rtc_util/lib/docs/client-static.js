...
$.KeywordState_computeKeywordStateTable = function(start, strings, offset, length$) {
  ...
  for (t1 = start + 1, t2 = strings.length, i = offset, chunkStart = -1, isLeaf = false, chunk = 0; t3 = offset + length$, i < t3; ++i) {
    ...
    if (typeof t3 !== 'number')
      return $.KeywordState_computeKeywordStateTable$bailout(1, start, strings, offset, length$, t3, result, t1, chunkStart, isLeaf, i, chunk, t2);
    if (t3 === start)
      isLeaf = true;
    t3 = strings[i];
    t3 = $.getInterceptor$JSStringJSArray(t3).get$length(t3);
    if (typeof t3 !== 'number')
      return $.KeywordState_computeKeywordStateTable$bailout(2, start, strings, offset, length$, result, isLeaf, t1, chunkStart, t3, i, chunk, t2);
    if (t3 > start) {
      t3 = strings[i];
      c = $.getInterceptor$JSString(t3).charCodeAt$1(t3, start);
      if (c !== (c | 0))
        return $.KeywordState_computeKeywordStateTable$bailout(3, start, strings, offset, length$, result, isLeaf, t1, chunkStart, i, c, chunk, t2);
      if (chunk !== c) {
        if (chunkStart !== -1) {
          t3 = chunk - 97;
